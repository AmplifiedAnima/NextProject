...
def TrainingPlanForUser(request,pk):
   ...
   #CASUALTRAINING
    ...
    #SQUAT1RM = TrainingPlan.objecs.values('Squat1RM').get(id=pk)
    exerintu= TrainingUnit.objects.all().filter(trainingplan=pk)
  
    day1firstexercise= exercises
    #Choices are: exercisename, exercisereps, exercisesets, from_signal, id, trainingunit, trainingunit_id
    ...
    if request.method =="POST":
        trainingunit = TrainingUnitForm(request.POST)
        if trainingunit.is_valid():
            trainingunit=TrainingUnit.objects.create(
            trainingplan=TrainingPlan.objects.get(id=pk),
...
def TrainingUnitInPlan(request,pk):
    ...
    exercises = trainingunit.exercise_set.all()
...
def TrainingPlanFreelance(request,pk):
    ...
    trainingunit= trainingplan.newbietrainingunit_set.all()
    context = {'trainingplan':trainingplan,'trainingunit': trainingunit}
...
def AddTrainingPlanFreelance(request):
    ...
    if request.method =="POST":
        ...
        return render(request,'core/addtrainingplanfreelance.html', context)
    else:
        return render(request, 'core/addtrainingplan.html', {"training": training})   
